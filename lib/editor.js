...
var MediumDraftEditor = function (_React$Component) {
  ...
  function MediumDraftEditor(props) {
    ...
    _this.handlePastedText = function (text, html, es) {
      ...
      if (currentBlock.getType() === _constants.Block.ACTIVITY) {
        var editorState = _this.props.editorState;
  ...
  _createClass(MediumDraftEditor, [{
    ...
    */
    ...
    value: function _toggleInlineStyle(inlineStyle) {
      ...
      if (inlineStyle === _constants.Inline.REMINDER) {
        var block = (0, _model.getCurrentBlock)(editorState);
        var characterList = block.getCharacterList().map(function (c) {
          return c.set('style', c.get('style').delete('REMINDER'));
        });
        ...
        var nextBlock = block.merge({
          characterList: characterList,
          data: data.merge({
            reminderText: selectedText,
            remindAt: (0, _addDays2.default)(new Date(), 3)
          })
        });

        this.onChange(_draftJs.RichUtils.toggleInlineStyle((0, _model.updateBlock)(editorState, nextBlock), inlineStyle));
      } else {
    ...
    */
    value: function render() {
      ...
      var editorClass = 'guided-editor' + (!editorEnabled ? ' guided-readonly' : '');
...
MediumDraftEditor.defaultProps = {
  ...
  continuousBlocks: [_constants.Block.UNSTYLED, _constants.Block.OL, _constants.Block.UL],
  sideButtons: [],
